...
void enqueue_drive_request(void* buf, size_t size, uint32_t lba_addr, RequestType type)
{
...
{
    return enqueue_drive_request(buf, size, lba_addr, READ);
}

void enqueue_drive_write(void* buf, size_t size, size_t lba_addr)
{
    return enqueue_drive_request(buf, size, lba_addr, WRITE);
}
